...
def stream_csv(file_path):
    ...
    for chunk in pd.read_csv(file_path, chunksize=chunk_size):
        ...
        # 👉 Bỏ cột cuối cùng của chunk hiện tại
        chunk_dropped = chunk.iloc[:, :-1]

        for index, row in chunk_dropped.iterrows():
            payload = row.to_dict()
        ...
        producer.flush()
        print(f"✅ Đã xử lý xong một cụm {chunk_size} dòng (đã bỏ cột cuối).")
